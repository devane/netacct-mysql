...
#endif
#ifdef HAVE_PGSQL
      	char *pgsql_user; /* by XerX, pgsql username in config file */
      	char *pgsql_password; /* by XerX, pgsql password in config file */
      	char *pgsql_host; /* by XerX, pgsql hostname in config file */
      	char *pgsql_database; /* by XerX, pgsql database in config file */
      	char *pgsql_table; /* by XerX, pgsql table in config file */ 
      	unsigned int pgsql_port; /* pgsql port in config file */ 
#endif
#ifdef HAVE_ORACLE
...
static struct HOST_DATA
{
	...
	char written;				/* This data is written to sql ? 0 - NO, 1 - YES */
...
pthread_t writting_thread;
pthread_t pt[MAX_INTERFACES];

int err_delay, max_err_delay;

static volatile int writting = 0;
static volatile int may_write = 1;

void* write_main_pgsql(void);
void* write_main_mysql(void);
void* write_main_oracle(void);

static int interface_number = 0;

#define WRITE_LOOP(A)  while(1) {						\
 if((may_write == 1) && (writting == 0)) { 					\
  if(err_delay != 0) {								\
    err_delay--;								\
    syslog(LOG_INFO,"flushing delayed due to error delay=%d", err_delay);	\
   } else {									\
    writting = 1;								\
    if (A != 0) {								\
	syslog(LOG_ERR,"SQL write error");					\
	err_delay = cfg->err_delay;						\
    }										\
    writting = 0;								\
   }										\
 }										\
