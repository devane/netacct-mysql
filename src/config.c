...
#ifdef HAVE_PGSQL
	cfg -> pgsql_user = NULL; 
	cfg -> pgsql_password = NULL;
	cfg -> pgsql_host = NULL;
	cfg -> pgsql_database = NULL;
	cfg -> pgsql_port = 5432; 
#endif
#ifdef HAVE_ORACLE
		...
		if(*buff) {
			...
			*value++ = '\0';
			...
			/* rweber: write to mysql or oracle or ... */
				...
				else if(!strcmp(cfg->database, "postgresql"))
					cfg->dbtype=TYPE_PGSQL;
				else {
...
#endif /* HAVE_MYSQL */
#ifdef HAVE_PGSQL
			/* set pgsql user from cfg file */
			else if(strcasecmp(kwd, "pgsql_user")==0)	{
				cfg->pgsql_user = strdup(value);
				syslog(LOG_DEBUG,"config: pgsql_user = %s\n",
						cfg->pgsql_user);
			}
			/* set pgsql passwd from cfg file */
			else if(strcasecmp(kwd, "pgsql_password")==0)	{
				cfg->pgsql_password = strdup(value);
				syslog(LOG_DEBUG,"config: password ok!");
			}
			/* set pgsql host from cfg file */
			else if(strcasecmp(kwd, "pgsql_host")==0)	{
				cfg->pgsql_host = strdup(value);
				syslog(LOG_DEBUG,"config: pgsql_host = %s\n",
						cfg->pgsql_host);
			}
			/* set pgsql database from cfg file */
			else if(strcasecmp(kwd, "pgsql_database")==0)	{
				cfg->pgsql_database = strdup(value);
				syslog(LOG_DEBUG,"config: pgsql DB = %s\n",
						cfg->pgsql_database);
			}
			/* set pgsql port from cfg file */
			else if(strcasecmp(kwd, "pgsql_port")==0)	{
				cfg->pgsql_port = atoi(value);
				syslog(LOG_DEBUG,"config: pgsql_port = %d\n",
						cfg->pgsql_port);
			}
#endif /* HAVE_PGSQL */
