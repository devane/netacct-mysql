...
/* Starting writting thread */
/* return values 0 - succeed, != 0 failed */
int init_writting(void) {    
	switch (cfg->dbtype) {
#ifdef HAVE_MYSQL
		case TYPE_MYSQL:
		    if(pthread_create(&writting_thread, NULL, &write_main_mysql, NULL) != 0) {
		    	syslog(LOG_ERR, "MySQL thread creation failed");
			return(1);		
		    } else {
		    	syslog(LOG_INFO, "MySQL writting thread started");
		    }
		    break;
#endif
#ifdef HAVE_PGSQL
		case TYPE_PGSQL:
		    if(pthread_create(&writting_thread, NULL, &write_main_pgsql, NULL) != 0 ) {
			syslog(LOG_ERR, "PostgreSQL thread creation failed");
			return(1);
		    } else {
		    	syslog(LOG_INFO, "PostgreSQL writting thread started");
		    }
		    break;
#endif
#ifdef HAVE_ORACLE
		case TYPE_ORACLE:
		    if(pthread_create(&writting_thread, NULL, &write_main_oracle, NULL) != 0 ) {
			syslog(LOG_ERR, "Oracle thread creation failed");
			return(1);
		    } else {
		    	syslog(LOG_INFO, "Oracle writting thread started");
		    }		    
		    break;
#endif
		default:
...
int main(int argc, char *argv[])
{
	...
	init_capture();
	if (init_writting() != 0) {
	    syslog(LOG_ERR, "Error starting writting thread");
	    exit(1);
	}
	/* start being useful */
